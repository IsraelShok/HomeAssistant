...
switcherIP = "192.168.1.126"
phone_id = "f7f6"
device_id = "59951f"
device_pass = "33313636"
...
"""
    ...
    print "Auto Extract the needed values for this script (device_id, phone_id and device_pass):" + " ./" + sys.argv[
        0] + " extract\r\n"
    sys.exit(1)
...
def GetSch(buffer, phone_id):
    ...
    if len(split) == 0:
        ...
        for i in range(len(split)):
            ...
            total_time = (
                        datetime.datetime.strptime(end_time, '%H:%M') - datetime.datetime.strptime(start_time, '%H:%M'))
    ...
    try:
        ...
        while True:
            ...
            if ba.hexlify(data)[0:4] != "fef0" and len(data) != 165:
                ...
                print "[*] Device is set to auto shutdown in: " + (
                            str(ihour) + " Hour(s) " + str(imin) + " Minute(s) " + str(isec) + " Second(s)")
...
if sys.argv[1] == "configure":
    ...
    try:
        ...
        while True:
            data, addr = sock.recvfrom(1024)
            if ((ba.hexlify(data)[0:4] != "fef0" and len(data) != 165)) or ((addr[0] != "192.168.1.1")) or (
            (addr[0] != "192.168.4.1")):
...
try:
    ...
    if sys.argv[1] == "0" and state == "0000":
        ...
        if sMinutes > 0 and sMinutes <= 60:
            ...
            data = "fef05d0002320102" + pSession2 + "340001000000000000000000" + getTS() + "00000000000000000000f0fe" + device_id + "00" + phone_id + "0000" + device_pass + "000000000000000000000000000000000000000000000000000000000106000" + sCommand + "00" + sTimer(
                sMinutes)
            data = crcSignFullPacketComKey(data, pKey)
            s.send(ba.unhexlify(data))
            res = s.recv(1024)
